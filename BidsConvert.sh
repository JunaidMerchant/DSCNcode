...
for s in ${SubID[@]}; do
  ...
  if [ ! -d $outdir/anat ]; then
    ...
    sed -e '/"InstitutionalDepartmentName": "Department",/a\  "IntendedFor": ["func/sub-'${subid}'_task-cmnt_run-01_bold.nii.gz", "func/sub-'${subid}'_task-cmnt_run-02_bold.nii.gz", "func/sub-'${subid}'_task-cmnt_run-03_bold.nii.gz", "func/sub-'${subid}'_task-cmnt_run-04_bold.nii.gz", "func/sub-'${subid}'_task-rest_run-01_bold.nii.gz"],' ${outdir}/fmap/sub-${subid}_dir-${Cur}_epi.json > ${outdir}/fmap/sub-${subid}_dir-${Cur}_epi.json.tmp && mv ${outdir}/fmap/sub-${subid}_dir-${Cur}_epi.json.tmp ${outdir}/fmap/sub-${subid}_dir-${Cur}_epi.json
		#
	done
	#
	#
